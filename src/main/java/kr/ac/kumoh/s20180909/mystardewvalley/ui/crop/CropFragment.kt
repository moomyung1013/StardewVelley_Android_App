...
class CropFragment : Fragment() {
    ...
    private lateinit var model: CropViewModel
    ...
    override fun onCreateView(
        ...
        model = ViewModelProvider(activity as AppCompatActivity)
            .get(CropViewModel::class.java)
        model.list.observe(viewLifecycleOwner,
            Observer<ArrayList<CropViewModel.Crop>> {
    ...
    inner class StardewAdapter: RecyclerView.Adapter<StardewAdapter.ViewHolder>() {
        inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView){
            val txName = itemView.findViewById<TextView>(R.id.text1) // 농작물 이름, 가격 출력
            val txModel = itemView.findViewById<TextView>(R.id.text2) //농작물 계절, 번들 출력
        ...
        override fun onCreateViewHolder(
            ...
            val view = layoutInflater.inflate(R.layout.item_crop, parent, false)
        ...
        override fun onBindViewHolder(holder: StardewAdapter.ViewHolder, position: Int) {
            ...
            holder.txModel.text = "$season 작물 / 번들${model.getStardew(position).bundle} "
